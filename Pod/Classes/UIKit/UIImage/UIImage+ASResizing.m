...
{
	return [self as_cropToSize:newSize usingMode:NYXCropModeTopLeft];
}
...
- (UIImage *)as_scaleByFactor:(float)scaleFactor
{
	...
	return [self as_scaleToFillSize:scaledSize];
}
...
- (UIImage *)as_scaleToSize:(CGSize)newSize usingMode:(NYXResizeMode)resizeMode
{
	switch (resizeMode)
	{
		case NYXResizeModeAspectFit:
			return [self as_scaleToFitSize:newSize];
		case NYXResizeModeAspectFill:
			return [self as_scaleToCoverSize:newSize];
		default:
			return [self as_scaleToFillSize:newSize];
	}
}
...
- (UIImage *)as_scaleToFillSize:(CGSize)newSize
{
  ...
  /// Create an ARGB bitmap context
	CGContextRef bmContext = ASCreateARGBBitmapContext(destWidth, destHeight, destWidth * kASNumberOfComponentsPerARBGPixel, ASImageHasAlpha(self.CGImage));
	if (!bmContext)
		return nil;
...
- (UIImage *)as_scaleToFitSize:(CGSize)newSize
{
	...
	return [self as_scaleToFillSize:CGSizeMake(destWidth, destHeight)];
}
