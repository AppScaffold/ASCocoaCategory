...
- (UIImage *)as_brightenWithValue:(float)value
{
	/// Create an ARGB bitmap context
	const size_t width = (size_t)self.size.width;
	const size_t height = (size_t)self.size.height;
	CGContextRef bmContext = ASCreateARGBBitmapContext(width, height, width * kASNumberOfComponentsPerARBGPixel, ASImageHasAlpha(self.CGImage));
	if (!bmContext) 
		return nil;
	...
	float min = (float)kASMinPixelComponentValue, max = (float)kASMaxPixelComponentValue;
...
#pragma unused(bias)
	...
	CGContextRef bmContext = ASCreateARGBBitmapContext(width, height, bytesPerRow, ASImageHasAlpha(self.CGImage));
	if (!bmContext) 
		return nil;
...
- (UIImage *)as_grayscale
{
	...
	CGContextRef bmContext = CGBitmapContextCreate(NULL, width, height, 8/*Bits per component*/, width * kASNumberOfComponentsPerGreyPixel, colorSpace, (CGBitmapInfo)kCGImageAlphaNone);
    ...
    CGImageRef grayImageRef = CGImageCreateWithMask(grayscaledImageRef, mask);
    UIImage *grayscaled = [UIImage imageWithCGImage:grayImageRef scale:self.scale orientation:self.imageOrientation];
...
- (UIImage *)as_opacity:(float)value
{
	...
	const size_t width = (size_t)self.size.width;
	const size_t height = (size_t)self.size.height;
	CGContextRef bmContext = ASCreateARGBBitmapContext(width, height, width * kASNumberOfComponentsPerARBGPixel, YES);
	if (!bmContext) 
		return nil;
...
- (UIImage *)as_sepia
{
	if ([CIImage class])
	{
		...
		CGImageRef cgImage = [ASGetCIContext() createCGImage:output fromRect:[output extent]];
